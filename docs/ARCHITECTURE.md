...
DBNotebook is a multimodal RAG (Retrieval-Augmented Generation) system with NotebookLM-style document organization. It provides intelligent document retrieval, persistent conversations, and multi-provider LLM support.

**Key Capabilities:**
- **RAG Chat**: Ask questions about your documents and get accurate, context-aware answers with source citations
- **SQL Chat**: Natural language to SQL with multi-database support (PostgreSQL, MySQL, SQLite)
- **Excel Analytics**: Upload Excel/CSV files and get AI-generated interactive dashboards with KPIs and charts
- **Multiple AI Models**: Choose between local AI (Ollama) or cloud providers (Groq, OpenAI, Anthropic, Gemini) based on your needs
...
- **Programmatic API**: REST API (`/api/query`) for scripting, automation, and external integrations
...
- **Multi-User Authentication**: User accounts with RBAC, API keys for programmatic access
...
```
PluginRegistry
├── LLM Providers       (groq, ollama, openai, anthropic, gemini)
...
```bash
LLM_PROVIDER=groq             # or ollama, openai, anthropic, gemini
...
```sql
-- Core Tables
users                    -- Multi-user accounts with password hash, role, API key
...
-- SQL Chat Tables
sql_connections         -- Database credentials (encrypted)
...
api_key                VARCHAR(64)       -- For programmatic access
```
...
db_type                VARCHAR(50)       -- postgresql|mysql|sqlite
encrypted_credentials  TEXT              -- Fernet encrypted

-- sql_sessions
session_id             UUID PRIMARY KEY
connection_id          UUID FOREIGN KEY → sql_connections
created_at             TIMESTAMP

-- sql_query_history
query_id               UUID PRIMARY KEY
session_id             UUID FOREIGN KEY → sql_sessions
...
## 11. SQL Chat (Text-to-SQL)

SQL Chat enables natural language queries against connected databases.
...
## 12. Excel Analytics (AI Dashboards)

Upload Excel/CSV files and get AI-generated interactive dashboards.
...
## 13. Programmatic API (`/api/query`)

REST API for scripting, automation, and external integrations.
...
| `/api/auth/regenerate-api-key` | POST | Generate new API key |
...
# Core Providers
# ============================================
LLM_PROVIDER=groq                      # groq|ollama|openai|anthropic|gemini
...
API_KEY=fallback-api-key               # Fallback for programmatic API
RBAC_STRICT_MODE=false                 # Enforce strict access control

# ============================================
# SQL Chat
# ============================================
SQL_CHAT_ENCRYPTION_KEY=fernet-key     # For credential encryption
SQL_CHAT_SKIP_READONLY_CHECK=false     # Set true for dev only
FEW_SHOT_MAX_EXAMPLES=100000           # Max SQL examples for RAG
```
...
└── core/                    # Business Logic Layer
    ...
    │   ├── groq.py          # Groq ultra-fast LLM (300-800 tok/s)
...
│
├── config/
│   ├── dbnotebook.yaml      # Unified configuration (ingestion, retrieval, RAPTOR, SQL Chat)
...
### 21.1 Adding a New LLM Provider
...
### 21.3 Adding a New Retrieval Strategy
